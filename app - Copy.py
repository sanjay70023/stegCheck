...
def weighted_ensemble_prediction(cnn_probs, chi_val, hist_difference, noise_metric, spam_probs, rs_metric, comp_diff_ratio, fourier_score, patch_noise_std, has_exif, estimated_payload_bytes):
    # Weights (Significantly reduced SPAM weight)
    weights = {
        'cnn': 0.25,
        'spam': 0.05,
        'chi': 0.12,
        ...
        'patch_noise': 0.08,
        'metadata': 0.05,
        'payload_est': 0.00 # Reduced payload weight as it seems influenced by Chi
    }
    ...
    # Histogram
    stego_likelihood_hist = min(max(hist_difference / 0.60, 0), 1) # Less aggressive
    hist_s = np.array([(1 - stego_likelihood_hist) * 0.7 + 0.3, stego_likelihood_hist, (1 - stego_likelihood_hist) * 0.5])

    noise_s = noise_score_for_ensemble(noise_metric)
    rs_s = rs_score_for_ensemble(rs_metric)

    # Compression: High ratio -> Strong Cover/Original
    stego_likelihood_comp = max(0, 1 - (comp_diff_ratio * 7))
    compression_s = np.array([stego_likelihood_comp * 0.6 + 0.4, 1 - stego_likelihood_comp, stego_likelihood_comp * 0.2])

    # Fourier: Low score -> Strong Cover/Original
    stego_likelihood_fourier = max(0, 1 - (fourier_score * 12))
    fourier_s = np.array([stego_likelihood_fourier * 0.7 + 0.3, 1 - stego_likelihood_fourier, stego_likelihood_fourier * 0.1])

    # Patch Noise: Low std -> Strong Cover/Original
    stego_likelihood_patch = max(0, 1 - (patch_noise_std * 400))
    patch_noise_s = np.array([stego_likelihood_patch * 0.7 + 0.3, 1 - stego_likelihood_patch, stego_likelihood_patch * 0.1])

    # Metadata
    metadata_s = np.array([0.7 if has_exif else 0.3, 0.3 if has_exif else 0.7, 0.5])

    # Payload Est (Reduced weight)
    stego_likelihood_payload = min(max(estimated_payload_bytes / 150000.0, 0), 1) # Slightly less sensitive
    payload_est_s = np.array([1 - stego_likelihood_payload, stego_likelihood_payload, 0.5 * (1 - stego_likelihood_payload)])

    st.markdown(f"CNN Score: {cnn_s}")
    st.markdown(f"SPAM Score: {spam_s}")
    st.markdown(f"Chi Score: {chi_s}")
    st.markdown(f"Hist Score: {hist_s}")
    st.markdown(f"Noise Score: {noise_s}")
    st.markdown(f"RS Score: {rs_s}")
    st.markdown(f"Compression Score: {compression_s}")
    st.markdown(f"Fourier Score: {fourier_s}")
    st.markdown(f"Patch Noise Score: {patch_noise_s}")
    st.markdown(f"Metadata Score: {metadata_s}")
    st.markdown(f"Payload Est Score: {payload_est_s}")
...
if uploaded_files:
    ...
    for uploaded_file_obj in uploaded_files:
        ...
        st.markdown(f"**Raw Histogram Difference:** `{hist_diff_metric}`")
        st.markdown(f"**Raw Compression Difference Ratio:** `{comp_diff_ratio_metric}`")
        st.markdown(f"**Raw Fourier Score:** `{fourier_score_metric}`")
        st.markdown(f"**Raw Patch Noise Std. Dev.:** `{patch_std_dev_noise}`")
        # --- Ensemble Prediction ---
        ...
        if ensemble_predicted_idx == 1: # Stego
            ...
            elif ensemble_confidence_score > 0.50:
                st.warning(f"⚠️ Conclusion: Moderate indication of **Stego Image**.")
            else:
                st.info(f"ℹ️ Conclusion: Weak indication of **Stego Image**.")
